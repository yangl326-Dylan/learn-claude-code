...
flowchart TB
    ...
    subgraph Loop["核心 Agent Loop"]
        B["cron/background 注入"]
        C["压缩管线<br/>(tool_result_budget → snip → micro → compact)"]
        D["system prompt 组装<br/>(身份 + 工具 + 记忆 + MCP 状态)"]
        E["LLM 调用<br/>(带错误恢复)"]
        F{"has tool_use?"}
        G["PreToolUse Hooks<br/>+ Permission 权限"]
        H["工具分发执行<br/>(内置 + MCP + 后台)"]
        I["PostToolUse Hooks"]
        J["tool_result → messages"]
        K["Stop Hooks → 返回"]
    end

    subgraph Knowledge["上下文与知识"]
        L["s07 skills<br/>(目录 + load_skill)"]
        M["s09 memory<br/>(MEMORY.md)"]
        N["s10 prompt<br/>(分段组装)"]
    end

    subgraph Governance["治理与扩展"]
        O["s03 permission<br/>(deny list + 审批)"]
        P["s04 hooks<br/>(4 种事件)"]
        Q["s11 error recovery<br/>(retry/fallback/compact)"]
    end

    subgraph Persist["持久工作"]
        R["s05 todo_write"]
        S["s12 task graph<br/>(.tasks/*.json)"]
        T["s13 background<br/>(daemon thread)"]
        U["s14 cron scheduler"]
    end

    subgraph Team["团队与插件"]
        V["s06 subagent<br/>(一次性隔离)"]
        W["s15-s17 team protocols<br/>(MessageBus + 协议)"]
        X["s18 worktree isolation<br/>(git worktree)"]
        Y["s19 MCP tools<br/>(后绑定)"]
    end

    subgraph ToolPool["TOOL POOL — 27 builtins + mcp__server__tool"]
        Z1["bash · read · write · edit · glob"]
        Z2["todo_write · task · load_skill · compact"]
        Z3["create/list/get/claim/complete_task"]
        Z4["schedule/list/cancel_cron"]
        Z5["spawn_teammate · send_message · check_inbox"]
        Z6["request_shutdown/plan/review_plan"]
        Z7["create/remove/keep_worktree · connect_mcp"]
    end
...
以下时序图展示一次完整用户交互中各组件的协作流程，包括后台任务和队友协作场景：
...
sequenceDiagram
    ...
    participant Hooks as Hooks/权限
    participant Tools as 工具处理器
    participant BG as 后台线程
    participant TM as 队友线程
    participant Cron as Cron 调度器

    Note over User,CLI: 主交互流程
    User->>CLI: 输入问题
    CLI->>Loop: history.append(query)
    Loop->>Loop: 压缩管线 (compact)
    Loop->>Loop: system prompt 组装
    Loop->>LLM: call_llm(messages, tools)

    alt 有 tool_use
        LLM-->>Loop: response (含 tool_use block)
        Loop->>Hooks: PreToolUse (permission 检查)
        Hooks-->>Loop: 通过/拒绝

        alt 后台操作 (build/install)
            Loop->>BG: start_background_task
            BG-->>Loop: placeholder tool_result
            BG->>BG: 在 daemon 线程执行...
            Note over BG: 完成后注入 notification
        else 普通工具
            Loop->>Tools: bash/read/write/edit
            Tools-->>Loop: tool_result
        else MCP 工具
            Loop->>Tools: mcp__server__tool
            Tools-->>Loop: MCP result
        end

        Loop->>Hooks: PostToolUse
        Loop->>Loop: results.append(tool_result)
        Loop->>Loop: 继续下一轮
    else 没有 tool_use
        LLM-->>Loop: text response (纯文本)
        Loop->>CLI: 显示回复
        CLI-->>User: 打印结果
    end

    Note over User,Cron: ⏰ 定时任务 & 队友协作
    Cron->>Cron: 每秒检查 cron 表达式
    alt cron 命中
        Cron-->>Loop: [Scheduled] 消息注入
        Loop->>LLM: 处理定时任务
    end

    alt 队友交互
        User->>CLI: "spawn alice as developer"
        CLI->>Loop: spawn_teammate
        Loop->>TM: 创建独立 daemon 线程
        TM->>TM: 独立 messages[] 循环
        User->>CLI: "ask alice to submit plan"
        CLI->>Loop: request_plan
        Loop->>TM: 发送 plan 请求消息
        TM-->>Loop: submit_plan (含 request_id)
        Loop->>CLI: plan 待审批
        User->>CLI: "approve the plan"
        CLI->>Loop: review_plan(approve=true)
        Loop->>TM: plan_approval_response
        TM->>TM: plan 批准后继续执行任务
        TM-->>Loop: 任务完成结果
        Loop-->>User: 显示队友状态
    end
```
